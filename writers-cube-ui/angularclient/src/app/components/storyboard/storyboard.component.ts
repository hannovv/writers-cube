...
export class StoryboardComponent implements OnInit {
  ...
  delete(plotpoint: Plotpoint) {
    ...
    this.plotPointService
      .findPlotPointByStoryboardId(this.storyboard.id)
      .subscribe((data) => {
  ...
  ngOnInit() {
    ...
    this.plotPointService
      .findPlotPointByStoryboardId(storyboardId)
      .subscribe((data) => {
        this.plotpoints = data;
      });
    this.startUpdating(storyboardId);
  }

  getPlotPoints(storyboardId: string) {
    this.plotPointService
      .findPlotPointByStoryboardId(storyboardId)
      .subscribe((data) => {
        this.plotpoints = data;
  ...
  startUpdating(storyboardId: string) {
    this.updateSubscription = interval(100).subscribe(() => {
      this.getPlotPoints(storyboardId);
    });
  }
}
